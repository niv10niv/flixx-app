...
//Display Backsrop On Details Pages
...
async function fetchAPIData(endpoint) {
  ...
  const response = await fetch(
    `${API_URL}${endpoint}?api_key=${API_KEY}&laguage-US`
  );
...
async function searchAPIData() {
  ...
  const response = await fetch(
    `${API_URL}search/${global.search.type}?api_key=${API_KEY}&laguage-US&query=${global.search.term}&page=${global.search.page}`
  );
